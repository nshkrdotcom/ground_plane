# GroundPlane Workspace Persistence

## Scope

GroundPlane Workspace owns shared persistence profile and lower infrastructure orientation. This document is package-local and is the persistence contract for `.` in `ground_plane`.
...
Memory/ref-only policy data by default; Postgres helpers are explicit lower infrastructure packages, not ambient defaults.
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. :integration_postgres with caller-supplied Postgres capability or :ops_durable with caller-supplied Temporal capability.
...
Durable adapters own their package-local migrations and preflight proof before mutation.
