# GroundPlane Persistence Policy Persistence

## Scope

GroundPlane Persistence Policy owns pure profile, tier, capture, store-set, partition, and debug tap contract owner. This document is package-local and is the persistence contract for `core/persistence_policy` in `ground_plane`.
...
Pure memory/ref-only policy data only. No Ecto, AshPostgres, Temporal, object store, network, product, or provider adapter dependency.
...
```bash
cd core/persistence_policy && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. :local_restart_safe, :integration_postgres, :ops_durable, :full_debug_tracked, or :distributed_partitioned only when caller capabilities are passed explicitly.
...
None in this package. Adapter packages own schema and migration preflight.
