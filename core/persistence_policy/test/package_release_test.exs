defmodule GroundPlane.PersistencePolicy.PackageReleaseTest do
  ...
  @required_files [
    ...
    "LICENSE",
    "README.md",
    "assets/ground_plane_persistence_policy.svg",
  ...
  test "project declares the public 0.1.0 package metadata" do
    ...
    assert project[:app] == :ground_plane_persistence_policy
    assert project[:version] == "0.1.0"
    assert project[:elixir] == "~> 1.19"
    assert package[:name] == "ground_plane_persistence_policy"
    assert package[:licenses] == ["MIT"]
    assert package[:maintainers] == ["nshkrdotcom"]

    assert package[:links] == %{
             "Changelog" =>
               "https://github.com/nshkrdotcom/ground_plane/blob/main/core/persistence_policy/CHANGELOG.md",
  ...
  test "docs identify the release source and package guides" do
    ...
    assert docs[:source_ref] == "ground_plane_persistence_policy-v0.1.0"
    assert docs[:source_url] == "https://github.com/nshkrdotcom/ground_plane"
    assert docs[:logo] == "assets/ground_plane_persistence_policy.svg"
  ...
  test "release presentation is package-local and branded" do
    ...
    logo = File.read!("assets/ground_plane_persistence_policy.svg")

    assert readme =~ ~s(src="assets/ground_plane_persistence_policy.svg")
