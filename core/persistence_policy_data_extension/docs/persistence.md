# GroundPlane Persistence Policy Data Extension Persistence

## Scope

GroundPlane Persistence Policy Data Extension owns data-class posture and redaction helper owner above the pure profile contract. This document is package-local and is the persistence contract for `core/persistence_policy_data_extension` in `ground_plane`.

## Available Tiers

- `:mickey_mouse`: memory or ref-only default. No restart durability claim.
- `:memory_debug`: memory or ref-only with redacted debug receipts only.
...
Memory/ref-only descriptors and redaction helpers only.

## Unsupported Adapters

Unsupported adapter selections fail before mutation. Silent fallback from durable selection to memory is invalid. Higher layers must not import lower store modules directly to compensate for a missing adapter.
...
```bash
cd core/persistence_policy_data_extension && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted receipt proof, focused tests, repo QC, and a pushed commit. Durable profile refs are accepted as proof only when supplied by the caller with capability proof.
...
No migrations. Durable substrates must preflight in their adapter package.
