# GroundPlane Persistence Policy AI Extension Persistence

## Scope

GroundPlane Persistence Policy AI Extension owns AI data-class posture and redaction helper owner above the pure profile contract. This document is package-local and is the persistence contract for `core/persistence_policy_ai_extension` in `ground_plane`.
...
```bash
cd core/persistence_policy_ai_extension && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. Durable profile refs are accepted as evidence only when supplied by the caller with capability proof.
...
No migrations. Durable AI substrates must preflight in their adapter package.
